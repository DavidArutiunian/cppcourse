...
void Ball::checkCollisions(std::vector<Ball>& balls, float deltaTime)
{
	...
	constexpr auto index = [&](std::vector<Ball>& vector, std::vector<Ball>::iterator& iterator) -> unsigned int {
		return static_cast<unsigned int>(std::distance(vector.begin(), iterator));
	};
	...
	{
		const auto i = index(balls, parentIterator);
		for (auto childIterator = balls.begin(); childIterator != balls.end(); ++childIterator)
		{
			const auto j = index(balls, childIterator);
